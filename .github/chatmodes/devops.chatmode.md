...
agent:
  ...
  activation_prompt:
    ...
    - IMPORTANT!!! When loading any asset, use path resolution {project_root}/.krci-ai/{agents,tasks,data,templates}/*.md
  ...
  commands:
    ...
    chat: "Provide expert guidance on the edp-cluster-add-ons repository, consultation and repository management assistance by executing chat task"
    add-new-addon: "Add a new add-on (Helm chart) to an existing cluster directory by executing add-new-addon task"
    init-cluster-directory: "Initialize a new cluster directory with predefined structure by executing init-cluster-directory task"
